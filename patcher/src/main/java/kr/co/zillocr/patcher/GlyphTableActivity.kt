...
/** PoC 2.3: dump full multibyte parser and keyed glyph lookup continuations. */
class GlyphTableActivity : ComponentActivity() {
    ...
    private val isoPicker = registerForActivityResult(ActivityResultContracts.OpenDocument()) { uri ->
        ...
        statusView.text = "multibyte parser + keyed glyph lookup 전체 분기 추적 중…"
        executor.execute {
            val report = try {
    ...
    private fun buildView(): ScrollView {
        ...
        root.addView(TextView(this).apply {
            text = "PoC 2.3 · keyed glyph lookup 전체 분기 추적\n2.2에서 0x1F9F8C가 16비트 parser 값과 노드의 u16 key를 직접 비교하는 실제 keyed lookup임이 확인됐습니다. 이전 tracer가 첫 early-return에서 함수를 끊는 문제가 있어, 이번 판은 multibyte parser와 lookup의 뒤쪽 분기를 강제로 끝까지 덤프합니다."
            textSize = 14f; setPadding(0, dp(10), 0, dp(14))
        })
        root.addView(Button(this).apply {
            text = "원본 ISO 선택 · full glyph lookup trace"
        ...
        root.addView(Button(this).apply {
            ...
            setOnClickListener {
                if (latestReport.isBlank()) Toast.makeText(this@GlyphTableActivity, "먼저 ISO 분석을 실행하세요.", Toast.LENGTH_SHORT).show()
                    ...
                    getSystemService(ClipboardManager::class.java).setPrimaryClip(ClipData.newPlainText("zill renderer glyph trace v8", latestReport))
