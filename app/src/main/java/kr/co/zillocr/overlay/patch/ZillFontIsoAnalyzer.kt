...
/** Extracts and authenticates the retail font/zillfont.par member from a clean PSP ISO. */
object ZillFontIsoAnalyzer {
    ...
    fun analyze(channel: SeekableByteChannel): Result {
        ...
        if (member.name != EXPECTED_FONT_NAME) {
            error("unexpected PAA member $FONT_MEMBER_INDEX: ${member.name}")
        }
        if (member.size != EXPECTED_FONT_SIZE) {
            error("unexpected zillfont.par size ${member.size}; expected $EXPECTED_FONT_SIZE")
        }
        if (member.offset + member.size > paArc.size) {
            error("zillfont.par extends outside pa.arc")
        }
    ...
    private fun parseMember(index: ByteArray, memberIndex: Int): PaaMember {
        ...
        if (memberIndex !in 0 until count) {
            error("PAA member $memberIndex out of range; count=$count")
        }
        ...
        return PaaMember(name = name, offset = offset, size = size)
    }
