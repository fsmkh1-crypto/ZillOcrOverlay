...
class Iso9660Reader(private val channel: SeekableByteChannel) {
    ...
    fun find(path: String): Entry {
        ...
        for ((index, part) in parts.withIndex()) {
            if (!current.isDirectory) {
                error("${parts.take(index).joinToString("/")} is not a directory")
            }
            ...
            if (next.isMultiExtent) {
                error("multi-extent ISO files are not supported yet: ${next.name}")
            }
            current = next
        }
        return current
    ...
    fun listDirectory(directory: Entry): List<Entry> {
        ...
        while (pos < data.size) {
            ...
            if (pos + recordLength > data.size || recordLength < 34) {
                error("invalid ISO9660 directory record at offset $pos")
            }
            val idLength = data[pos + 32].toInt() and 0xff
            if (33 + idLength > recordLength) {
                error("invalid ISO9660 file identifier at offset $pos")
            }
