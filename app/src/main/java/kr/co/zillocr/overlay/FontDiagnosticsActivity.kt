...
import kr.co.zillocr.overlay.patch.ZillFontIsoAnalyzer
...
class FontDiagnosticsActivity : ComponentActivity() {
    ...
    private val isoPicker = registerForActivityResult(ActivityResultContracts.OpenDocument()) { uri ->
        ...
        try {
            ...
            // Some providers grant temporary access only; analysis still works for this session.
        }
    ...
    private fun buildContentView(): ScrollView {
        ...
        root.addView(TextView(this).apply {
            text = "정품 ULJM-05410 v1.03 ISO를 선택하면 앱이 ISO 내부의 pa.bin/pa.arc를 읽고 font/zillfont.par(멤버 13611)을 직접 추출·SHA-256 검증합니다. 원본 ISO는 수정하거나 업로드하지 않습니다."
            textSize = 14f
        ...
        root.addView(Button(this).apply {
            ...
            setOnClickListener {
                if (latestReport.isBlank()) {
                    Toast.makeText(this@FontDiagnosticsActivity, "먼저 ISO 분석을 실행하세요.", Toast.LENGTH_SHORT).show()
                } else {
                    val clipboard = getSystemService(ClipboardManager::class.java)
                    clipboard.setPrimaryClip(ClipData.newPlainText("zillfont diagnostics", latestReport))
                    Toast.makeText(this@FontDiagnosticsActivity, "분석 결과를 복사했습니다.", Toast.LENGTH_SHORT).show()
                }
            }
        }, ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT)
